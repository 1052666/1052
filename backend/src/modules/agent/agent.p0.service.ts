...
function getRoutingPrompt() {
  return [
    ...
    '- IMPORTANT: The Skill index is injected in your context. If the current user task matches or relates to ANY installed Skill by name or description, you MUST request skill-pack, then call skills_read to read the full SKILL.md, and follow the Skill workflow. Do not skip installed Skills and attempt to solve the task with general tools when a matching Skill exists.',
    '- When the user explicitly asks to use, run, or apply a Skill, always request skill-pack first, call skills_list if needed, then skills_read to get the full instructions before acting.',
    '- When the user describes a reusable workflow, analysis framework, task template, or capability module, proactively suggest creating a Skill via skills_create (in skill-pack). Do not just give text advice — offer to create the Skill.',
